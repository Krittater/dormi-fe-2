...
export function ApartmentOverviewPage() {
  ...
  const kpis = useMemo<Kpi[]>(
    ...
    [available, overdueAmount, overdueInvoices.length, rented, t, total]
  );
  ...
  const shortcuts = useMemo(
    () => [
      ...
      {
        label: t("nav-billing-periods"),
        desc: t("link-billing-periods-desc"),
        icon: FileText,
        seg: "billing-periods",
      },
  ...
  const notifications = useMemo(
    ...
    [dueSoon.length, overdueAmount, overdueInvoices.length, openPeriods.length, t]
  );

  const go = useCallback(
    (seg: string) => router.push(`/apartments/${apartmentId}/${seg}`),
    [apartmentId, router]
  );
